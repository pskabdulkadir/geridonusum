...
async function broadcastToGreenFinanceNetwork(proof: any) {
    try {
        ...
        const OCEAN_CONTRACT = process.env.OCEAN_MARKET_CONTRACT || "0x8967b... (Ocean Market tarafından size verilen adres)"; // Bu adresi .env'den çekmek daha güvenli olacaktır.
        ...
        while (attempts < maxRetries && !success) {
            attempts++;
            try {
                // provider.oceanprotocol.com yerine v4.provider.oceanprotocol.com kullanıldı
                response = await axios.post("https://v4.provider.oceanprotocol.com/api/v1/services/publish", {
                    metadata: {
                ...
                if (attempts < maxRetries) {
                    ...
                    await new Promise(resolve => setTimeout(resolve, 3000)); // 3 saniye bekle ve yeniden dene
                } else {
        ...
        // error.code veya error.hostname gibi alanları kontrol edilebilir.
